...
class DashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Allow request query override, otherwise fallback to settings in DB
        $matrixX = $request->query('matrix_x_param') ?: Setting::where('key', 'matrix_x_param')->value('value') ?: 'payment_delay';
        $matrixY = $request->query('matrix_y_param') ?: Setting::where('key', 'matrix_y_param')->value('value') ?: 'dti';
    ...
    /**
     * Save matrix default axis settings.
     */
    public function updateMatrixSettings(Request $request)
    {
        ...
        Setting::updateOrCreate(['key' => 'matrix_x_param'], ['value' => $request->matrix_x_param]);
        Setting::updateOrCreate(['key' => 'matrix_y_param'], ['value' => $request->matrix_y_param]);

        return response()->json([
            'message' => 'Pengaturan default diagram matriks berhasil disimpan!'
        ]);
    }
    ...
    {
        $parameterX = RiskParameter::where('key', $paramX)->first();
        $parameterY = RiskParameter::where('key', $paramY)->first();
