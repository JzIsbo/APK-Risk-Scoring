...
class PopulationController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $populations = $query->orderBy('created_at', 'desc')->paginate(10)->withQueryString();

        return view('populasi', compact('populations'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('populasi.index')->with('success', 'Data debitur berhasil ditambahkan dan tingkat risiko dihitung secara real-time!');
    }
    ...
    public function update(Request $request, Population $population)
    {
        ...
        return redirect()->route('populasi.index')->with('success', 'Data debitur berhasil diperbarui!');
    }
    ...
    {
        $population->delete();
        return redirect()->route('populasi.index')->with('success', 'Data debitur berhasil dihapus!');
    }
     ...
     * Useful when weights/criteria parameters are updated by Admin.
     */
    public function recalculateAll()
    {
        ...
        return redirect()->route('populasi.index')->with('success', 'Semua data debitur berhasil di-kalkulasi ulang berdasarkan parameter terbaru!');
    }
    ...
    public function importExcel(Request $request)
    {
        ...
        try {
            ...
            if (count($rows) <= 1) {
                return back()->withErrors(['file' => 'Berkas Excel kosong atau tidak memiliki baris data.']);
            }
            ...
            foreach ($essentialFields as $field) {
                if ($mappedIndexes[$field] === -1) {
                    return back()->withErrors(['file' => "Kolom penting '{$field}' tidak dapat dicocokkan di file Excel. Pastikan nama header sesuai."]);
                }
            }
            ...
            return back()->with('success', "Berhasil mengimpor {$successCount} data debitur dari Excel. Seluruh risiko telah dikalkulasi ulang secara real-time!");

        } catch (\Exception $e) {
            return back()->withErrors(['file' => 'Gagal membaca berkas Excel: ' . $e->getMessage()]);
        }
    }
}
