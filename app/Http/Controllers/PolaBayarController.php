...
class PolaBayarController extends Controller
{
    public function index(Request $request)
    {
        ...
        $crossTab = Population::select('payment_pattern', 'ps_ambc', DB::raw('count(*) as cnt'))
            ->groupBy('payment_pattern', 'ps_ambc')
            ->get()
            ->groupBy('payment_pattern');

        return view('pola-bayar', compact(
            'periods', 'periodLabels',
            'polaBayarData', 'psAmbcData',
            'avgDelayPerPeriod',
            'patternSummary', 'psAmbcSummary',
            'noOrderByPattern', 'crossTab'
        ));
    }
}
